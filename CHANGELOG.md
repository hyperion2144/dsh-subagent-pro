...
- **`subagent_roles` now lists md-loaded roles** — the runtime role table was fed by a cwd-only agent md scan (`shell.cwd/.dsh/agents`), so the agent-facing tool, delegation lookup and system-prompt guidance saw no file-backed roles when the shell cwd had no `.dsh/agents` (while the settings UI, which uses the multi-workspace bridge loader, did). The host now fills the role table from `loadAgentMdRolesAcrossWorkspaces` — the same loader the bridge uses (every registered workspace's `.dsh/agents/*.md` + global dir, project wins on id collisions) — with the documented shell-cwd fallback for headless/smoke profiles without a workspace registry.
  - The registry is read through `ctx.get('workspaceRegistry')` — the cordis optional-service read that does NOT require the service in the entry's `inject` (declaring it would hang boot in headless profiles where the service is absent; direct property access throws "cannot get property without inject"). `ctx.get` also resolves sibling-entry services, so the loader stays correct for both the host entry and the bridge.
  - The md layer is re-scanned on cordis's `internal/service` event for `workspaceRegistry`: dsh-workspace's `Service.init` bootstrap is async, so a scan at `apply()` time can run before the registry is active. New wiring tests mount the real `apply()` on a cordis-faithful mock ctx and drive the real `subagent_roles` tool: workspace roles listed, cwd fallback, empty case, and late-registry rescan.
