...
function mountMonitor(ctxIn, _resolved) {
	...
	if (ctx.webServer !== void 0) ctx.webServer.register({
		kind: "exact",
...
function applyDefaultRouteSeam(ctx, getSettings) {
	...
	subagents.start = (name, request) => {
		...
		if (agentOptions !== void 0) {
			...
			return originalStart.call(subagents, name, {
				...request,
				agentOptions
			});
		}
...
function createDelegationTool(opts) {
	...
	return defineTool({
		...
		async execute(args, exec) {
			...
			ctx.logger.info("[dsh-subagent-pro] delegate layer=" + route.layer + " mode=" + (continuable ? "continuable" : "one-shot") + " transport=" + providerName + " route=" + JSON.stringify(route.agentOptions ?? null) + " persona=" + (route.persona ? "yes" : "no") + " warnings=" + JSON.stringify(warnings));
					...
					jobId: jobs.start({
						...
						run: () => {
							...
							return {
								...
								done: settleBackgroundRun(ctx.subagents.start(providerName, {
									...request,
...
function resolveSettings(ctx, config) {
	...
	if (settingsSvc === void 0) {
		ctx.logger?.debug?.("[subagent-pro] no settings service mounted; using empty base");
		return {
			get: () => base,
	...
	try {
		...
		for (const d of descriptors) if (d.ns === "subagent-pro") cached = d.value ?? {};
	} catch (err) {
		ctx.logger?.warn?.("[subagent-pro] settings describe failed: " + (err instanceof Error ? err.message : String(err)));
	}
	base = cached;
	const onChange = ctx.on;
	onChange?.("settings/change", (ns, next) => {
...
function apply(ctx, config = {}) {
	...
	reloadMd();
	ctx.on("settings/change", reloadMd);
