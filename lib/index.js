...
function invalidProviderError(provider, available) {
	...
	return /* @__PURE__ */ new Error(ERROR_PREFIX + " LLM provider route " + provider + " is not routable (no adapter serves it). Available providers: " + list);
}
...
function createDelegationTool(opts) {
	...
	return defineTool({
		...
		async execute(args, exec) {
			...
			if (route.persona !== void 0 && !provider.capabilities.persona) throw new Error(ERROR_PREFIX + " role binds a persona but transport provider \"" + providerName + "\" does not support the persona capability — switch the subagent provider or drop the role persona");
			if (route.toolFilter !== void 0 && !provider.capabilities.toolFilter) throw new Error(ERROR_PREFIX + " role binds a tool filter but transport provider \"" + providerName + "\" does not support the toolFilter capability — switch the subagent provider or drop the role filter");
			if (typeof maxDepth === "number" && !provider.capabilities.depthLimit) throw new Error(ERROR_PREFIX + " transport provider \"" + providerName + "\" cannot enforce maxDepth (no depthLimit capability) — set maxDepth: \"provider-managed\" to leave the recursion budget to the provider");
			...
			if (runInBackground && continuable) {
				if (provider.prepareContinuable === void 0) throw new Error(ERROR_PREFIX + " transport provider \"" + providerName + "\" does not support backgroundMode: continuable — switch the subagent provider or use backgroundMode: \"one-shot\"");
			...
			if (runInBackground) {
				...
				if (jobs === void 0) throw new Error(ERROR_PREFIX + " background jobs unavailable: load @deepseek-ai/dsh-jobs and @deepseek-ai/dsh-tool-jobs");
				return {
					kind: "background",
					jobId: jobs.start({
						kind: "subagent",
...
function validateLlmInfoArgs(args) {
	if (args.model !== void 0 && args.model !== "" && (args.provider === void 0 || args.provider === "")) return "dsh-subagent-pro: argument \"model\" requires a non-empty \"provider\" argument";
}
...
*/
async function buildProviderCatalog(llm, scopedProvider, scopedModel) {
	const providers = llm.listProviders().filter((p) => scopedProvider === void 0 || p.id === scopedProvider).map(toProviderEntry);
	const out = [];
	for (const provider of providers) {
		...
		const models = rawModels.filter((m) => scopedModel === void 0 || m.id === scopedModel).map(toModelEntry);
		const modelEntries = [];
...
function createLlmInfoTool(opts) {
	...
	return defineTool({
		name: toolName,
		description: "Discover routable LLM providers, models, and reasoning-effort levels, organized as a nested catalog: each provider contains its models, each model contains its supported reasoning efforts and the adapter default. All arguments are optional: call with no arguments to get every provider; pass `provider` to scope to that provider only; pass `provider` + `model` to get only that exact model leaf. Tolerates a missing `llm` service: returns an empty catalog rather than throwing.",
		parameters: {
			provider: {
				type: "string",
		...
		output: {
			...
			render: (_args, value) => {
				const lines = [];
				if (value.providers.length === 0) lines.push("no providers available");
				for (const p of value.providers) {
					lines.push(p.id + (p.name === p.id ? "" : " (" + p.name + ")"));
					if (p.models.length === 0) lines.push("  (no models)");
					for (const m of p.models) {
						const effortText = m.reasoningEfforts.length === 0 ? "no reasoning efforts" : m.reasoningEfforts.map((e) => e.id + (e.name === e.id ? "" : " (" + e.name + ")")).join(", ");
						lines.push("  " + m.id + (m.name === m.id ? "" : " (" + m.name + ")") + ": efforts=[" + effortText + "] default=" + (m.defaultEffort ?? "(none)"));
					}
				}
				return [{
					type: "text",
		...
		async execute(args) {
			...
			const provider = args.provider === "" ? void 0 : args.provider;
			const model = args.model === "" ? void 0 : args.model;
			const llm = getLlmInfoService(ctx);
			if (llm === void 0) return {
				kind: "catalog",
				providers: []
			};
			ctx.logger?.info?.("[dsh-subagent-pro] catalog scoped provider=" + (provider ?? "") + " model=" + (model ?? ""));
			return {
				kind: "catalog",
...
function toRoleEntry(role, id) {
	return {
		...
		...role.persona !== void 0 && role.persona !== "" ? { hasPersona: true } : {},
		...role.toolFilter?.allow !== void 0 ? { toolAllow: role.toolFilter.allow } : {},
		...role.isOverride === true ? { isOverride: true } : {}
	};
}
function createRolesTool(opts) {
	const { getRoles } = opts;
	const toolName = opts.toolName ?? "subagent_roles";
	return defineTool({
		name: toolName,
		description: "List every role that can be passed to the `subagent_role` tool, merged from project agent md files (each registered workspace’s .dsh/agents/*.md), global agent md files (~/.dsh/agents/*.md), and the subagent-pro settings namespace. Call with no arguments to list all roles; pass `role` (id or display name) to get one role's details. Each entry includes its source label, optional provider/model route binding, whether a persona is attached, and tool scoping.",
		parameters: { role: {
			type: "string",
		...
		output: {
			...
			render: (_args, value) => {
				...
				return [{
					type: "text",
					text: value.roles.map((r) => {
						const route = r.provider !== void 0 || r.model !== void 0 ? " [" + [r.provider, r.model].filter((s) => s !== void 0).join("/") + "]" : "";
						const persona = r.hasPersona === true ? " persona" : "";
						const tools = r.toolAllow !== void 0 ? " tools=" + r.toolAllow.join(",") : "";
		...
		isConcurrencySafe: () => true,
		async execute(args) {
			const entries = getRoles().map((r) => toRoleEntry(r));
			return {
				kind: "roles",
				roles: args.role === void 0 || args.role === "" ? entries : entries.filter((r) => r.id === args.role || r.displayName === args.role)
			};
		}
	});
}
...
function mountRoles(ctx, config, resolved, _agentMd) {
	...
	if (config.enableRolesTool !== false) {
		...
		const rolesTool = createRolesTool({
			ctx,
			toolName: rolesToolName,
			getRoles: resolved.getRoles
		});
		ctx.tools.register(rolesTool);
	}
...
function apply(ctx, config = {}) {
	...
	const reloadMd = () => {
		const merged = loadAgentMdRolesAcrossWorkspaces(ctx, resolved.globalAgentDir, resolved.projectAgentDirName);
